...
if st.session_state.page == "food":
    ...
    new_unit = st.selectbox("単位", UNITS)

    f_k = st.number_input("1単位あたりのカロリー", min_value=0.0)
    f_p = st.number_input("1単位あたりのたんぱく質", min_value=0.0)
    f_f = st.number_input("1単位あたりの脂質", min_value=0.0)
    f_c = st.number_input("1単位あたりの炭水化物", min_value=0.0)
