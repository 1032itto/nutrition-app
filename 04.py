...
def load_log():
    try:
        ...
        return pd.DataFrame(columns=["date", "meal", "foods", "kcal", "protein", "fat", "carbs"])
...
for meal, vals in st.session_state.meals.items():
    ...
    if "foods_added" in st.session_state:
        for i, f in enumerate(st.session_state.foods_added.get(meal, [])):
            col1, col2 = st.columns([4,1])
            col1.write(f"・ {f}")
            if col2.button("削除", key=f"del_{meal}_{i}"):
                food_name, amount = f.rsplit(" ", 1)
                amount = float(amount.replace("g", ""))
                row = food_db[food_db["food"] == food_name].iloc[0]
                st.session_state.meals[meal]["kcal"] -= row["kcal"] * amount / 100
                st.session_state.meals[meal]["protein"] -= row["protein"] * amount / 100
                st.session_state.meals[meal]["fat"] -= row["fat"] * amount / 100
                st.session_state.meals[meal]["carbs"] -= row["carbs"] * amount / 100
...
if st.button("CSVに保存する"):
    ...
    new_rows = []
    for meal, vals in st.session_state.meals.items():
        foods_str = ", ".join(st.session_state.foods_added.get(meal, []))
        new_rows.append([today, meal, foods_str, vals["kcal"], vals["protein"], vals["fat"], vals["carbs"]])([today, meal, vals["kcal"], vals["protein"], vals["fat"], vals["carbs"]])
