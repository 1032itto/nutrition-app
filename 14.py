...
if st.session_state.page == "weekly":
    ...
    st.subheader("推移グラフ")
    chart_df = daily[["kcal", "protein", "fat", "carbs"]].copy()

    # 目標線追加
    chart_df["kcal_target"] = st.session_state.targets["kcal"]
    chart_df["protein_target"] = st.session_state.targets["protein"]
    chart_df["fat_target"] = st.session_state.targets["fat"]
    chart_df["carbs_target"] = st.session_state.targets["carbs"]
