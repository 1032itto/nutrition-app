...
if st.session_state.page == "settings":
    ...
    p["height"] = st.number_input("身長 (cm)", 140, 200, p["height"])
    p["weight"] = st.number_input("体重 (kg)", 30.0, 150.0, p["weight"])
    p["goal"] = st.selectbox(
        "目的",
        ["減量", "ダイエット", "維持", "筋肥大"],
...
for meal in ["朝", "昼", "夜"]:
    for k in total:
        total[k] += st.session_state.meals[meal][k]
...
st.markdown(
    f"""
    **目標との差**
    - kcal : {total['kcal'] - t['kcal']:+.1f}
    - P : {total['protein'] - t['protein']:+.1f}
    - F : {total['fat'] - t['fat']:+.1f}
    - C : {total['carbs'] - t['carbs']:+.1f}
    """
)
