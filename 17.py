        ...
        if food_db.empty:
            ...
            if view_df.empty:
                ...
                if "edit_index" in st.session_state:
                    ...
                    e_kcal = st.number_input(f"{per_label_edit}のカロリー", value=float(row["kcal"]), key="edit_kcal")
                    e_p = st.number_input(f"{per_label_edit}のたんぱく質", value=float(row["protein"]), key="edit_p")
                    e_f = st.number_input(f"{per_label_edit}の脂質", value=float(row["fat"]), key="edit_f")
                    e_c = st.number_input(f"{per_label_edit}の炭水化物", value=float(row["carbs"]), key="edit_c")
